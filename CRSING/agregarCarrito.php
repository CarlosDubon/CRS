	...
	<body role="document">
			...
			<div class="panel-body">
				...
				<form action=<?= "'agregarCarrito.php".$var?> method="POST" role="form">					
					...
					<div class="input-group">						
						...
						<span class="input-group-btn"><button type="submit" class="btn btn-default">View</button></span> 
					</div>
                <div class="alignbutton" >
                 <a id="btnver" href="verCarrito.php" class="btn btn-default">View Reservations</a>
               </div>
				</form>
				</div>
				  ...
				  try{
				       ...
				       /* for ($i=0; $i <4 ; $i++) { 
				        ...
				        foreach ($result as $row) {
				        	...
				        	echo '<tr> <td>';
				            echo 'Schedule: '.$row['inicio'].' - '. $row['fin'].'<br /> Area:'. $lugar;
				            //echo $row['espacio_idespacio'];
				            echo"<br /><a href='verCarrito.php?id= ".$row['idhorario']."'>Reserve</a>";
				            if($_SESSION['tipo']==1)
                        	{ 
                        		echo"<br /><a href='agregarCarrito.php?delete=".$row['idhorario']."'>Delete</a>";
		...
		<script type="text/javascript">
		 //$(document).ready(function() {
			//$('#hinicio').clockpicker();
			$('#fecha').datetimepicker({ minDate:0,lang:'en',format:'Y-m-d',timepicker:false, mask:true});
        ...
        <a id="btnreg" href="areas.php" class="btn btn-warning">Back</a>
	</body>
	</html>
	<?php 
